...
export default function Dashboard({
  ...
  // "Apply" in the navigation lands here with ?apply=1 from any screen
  useEffect(() => {
    if (autoOpen) setPicking(true);
  }, [autoOpen]);

  /** Drop ?apply=1 so the button works again on the next press. */
  function clearApplyParam() {
    if (autoOpen) router.replace('/', { scroll: false });
  }
